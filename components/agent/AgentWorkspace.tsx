...
import { Brain, Download, Eye, FileCode2, FolderOpen, Play, Plus, Terminal, Trash2, X } from "lucide-react";
...
type Tab = "preview" | "code" | "files" | "console" | "memory";
...
export function AgentWorkspace({
  ...
  // Held in a ref so a parent re-render cannot retrigger the file-open effect.
  ...
  useEffect(() => {
    if (!focusFile) return;
    const language = LANGUAGE_FOR[focusFile.language] || "javascript";
    const code = focusFile.content || "";
    setDraft({ name: focusFile.name, language, code });
    setTab(focusFile.language === "html" || focusFile.language === "svg" ? "preview" : "code");
  ...
  async function runDraft() {
    ...
    if (!handle) return;
    setRunning(true);
    onLog({ level: "info", text: `▶ running ${draft.name}` });
  ...
  return (
    <aside className={`${open ? "flex" : "hidden"} h-full flex-col lg:flex`}>
      <div className="a-glass flex h-full flex-col overflow-hidden rounded-[26px]">
        <div className="flex items-center gap-1 border-b border-line px-2 py-2">
          ...
          <button onClick={onClose} className="rounded-full p-1.5 text-muted hover:bg-white" aria-label="Close workspace">
            <X className="h-4 w-4" />
          </button>
        </div>

        {/* The sandbox runner must stay mounted at all times. */}
        <div className="relative min-h-0 flex-1">
          ...
          {tab === "preview" && !preview && (
            <div className="absolute inset-0 grid place-items-center p-6 text-center">
              <div>
                ...
                <p className="a-mono mt-1 max-w-xs text-[11px] text-muted">
                  Isolated iframe (opaque origin, no cookies, no storage). Ask the agent to build something, or switch to Code and run it yourself.
                </p>
              </div>
            </div>
          )}
          ...
          {tab === "code" && (
            <div className="absolute inset-0 flex flex-col">
              <div className="flex flex-wrap items-center gap-2 border-b border-line px-3 py-2">
                ...
                >
                  {["javascript", "html", "python", "css", "svg", "json"].map((l) => (
                ...
                <button className="a-chip" onClick={() => onPreview({ language: draft.language, code: draft.code })}>
                    ...
                    <button
                      ...
                      onClick={async () => {
                        ...
                        if (json.file) {
                          setDraft({
                            name: json.file.name,
                            language: LANGUAGE_FOR[json.file.language] || "javascript",
          ...
          {tab === "console" && (
            <div className="absolute inset-0 flex flex-col">
              <div ref={consoleRef} className="a-mono a-scroll min-h-0 flex-1 overflow-auto bg-[#0a1020] p-3 text-[11px] leading-5 text-[#d7e0ec]">
                {logs.length === 0 && <div className="text-white/40">Sandbox console - stdout, errors and run timings land here.</div>}
                {logs.map((l, i) => (
                  <div key={i} className={l.level === "error" ? "text-[#ff8fa3]" : l.level === "warn" ? "text-[#ffd479]" : ""}>
                    {l.text}
                  </div>
                ))}
              </div>
  ...
  return (
    <div className="a-scroll absolute inset-0 overflow-auto p-3">
      <p className="mb-2 text-[11px] text-muted">
        Long-term memory. Facts survive restarts and are injected into every future turn - this is how the agent evolves.
      </p>
